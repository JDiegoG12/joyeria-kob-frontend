...
/**
 * Resuelve una categoría de producto al mismo esquema jerárquico usado por
 * el selector de dos pasos y por los filtros del panel admin.
 *
 * - Si la categoría es raíz: `selectedParentId = id` y `selectedSubId = null`
 * - Si la categoría es hija: `selectedParentId = parentId` y `selectedSubId = id`
 */
export const resolveCategorySelection = (category: {
  id: number;
  parentId: number | null;
}) => ({
  selectedParentId: category.parentId ?? category.id,
  selectedSubId: category.parentId === null ? null : category.id,
});
